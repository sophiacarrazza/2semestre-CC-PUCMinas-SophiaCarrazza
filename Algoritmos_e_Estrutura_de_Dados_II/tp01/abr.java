class abr{ //algebra booleana com as funcoes for dos metodos Or, And e Not recursivos

public static boolean Or(String x, int index) {
    if (index >= x.length()) { //se o index > q length, entao percorremos a string
        return false;
    }
...
public static boolean And(String x, int index) {
    if (index >= x.length()) {
        return true;
    }
    ...
    return And(x, index+1);
}

public static boolean Not(String x, int index) {
    if (index >= x.length()) {
        return true; 
    }
...
public static boolean ResolverEquacao(String x) {
    ...
    while(compacta.contains("(")){ //enquanto tiver paranteses abertos a expressao nao acabou
        ...
        switch (operador) {
            case 'd':
                resaux = And(subx, 0); //chamando a funcao p avaliar a subexpressao
                break;
            case 'r':
                resaux = Or(subx, 0);
                break;
