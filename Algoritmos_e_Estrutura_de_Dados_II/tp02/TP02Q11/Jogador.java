...
public void countingsort(ArrayList<Jogador> lista) {
    int maiorAltura = getMaior(lista).getAltura();

    // array para contar o número de ocorrências de cada altura
    ...
    // conta as alturas e armazena no array de contagem
    for (Jogador jogador : lista) {
        int altura = jogador.getAltura();
        count[altura]++;
    }

    // cria um novo ArrayList para armazenar os elementos ordenados
    ArrayList<Jogador> ordenado = new ArrayList<>(lista.size());

    // preenche o ArrayList ordenado com base nas contagens
    for (int i = 0; i < count.length; i++) {
        while (count[i] > 0) {
            for (Jogador jogador : lista) {
                if (jogador.getAltura() == i) {
                    ordenado.add(jogador);
                    count[i]--;
                }
            }
        }
    ...
    // agora a lista ordenado contém jogadores ordenados por altura

    // ordenação estável p/ ordenar pelo nome quando as alturas são iguais
    Collections.sort(ordenado, new Comparator<Jogador>() {
        @Override
...
public Jogador getMaior(ArrayList<Jogador> lista) {
    int maiorAltura = 0;
    String maiorNome = "";
    Jogador maior = lista.get(0);

    for (Jogador jogador : lista) {
        int altura = jogador.getAltura();
        String nome = jogador.getNome();
        if (altura > maiorAltura || (altura == maiorAltura && nome.compareTo(maiorNome) > 0)) {
            maiorAltura = altura;
            maiorNome = nome;
            maior = jogador;
        }
    ...
    return maior;
		...
		try {
            FileWriter escrever = new FileWriter("00817611_countingsort.txt");
