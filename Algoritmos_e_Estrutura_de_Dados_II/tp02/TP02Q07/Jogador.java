...
class Jogador{
        ...
        for (int i = 0; i < n; i++) {
			...
			int anochave = array.get(i).getAnoNascimento();
			String nchave = array.get(i).getNome();
            int j = i - 1;
            
            while (j >= 0) {
            Jogador jogadorAtual = array.get(j);
            int comp = Integer.compare(jogadorAtual.getAnoNascimento(), anochave);
            if (comp < 0 || (comp == 0 && jogadorAtual.getNome().compareTo(nchave) <= 0)) {
                break;
            }
            array.set(j + 1, jogadorAtual);
            j--;
        }
		...
		try {
            FileWriter escrever = new FileWriter("00817611_insercao.txt");
