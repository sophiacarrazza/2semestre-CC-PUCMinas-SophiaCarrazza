...
void ordSelecao(Jogador lista[], int n, int i) {
        ...
        //for (int i = 0; i < n - 1; i++) {
            ...
            Jogador temp = lista[i]; // temp é o menor jogador encontrado
            lista[i] = lista[menorIndice]; //i = menor indice da lista
            lista[menorIndice] = temp; // menor indice é temp
        //}
...
void quicksort(Jogador lista[], int esquerda, int direita) {
    if (esquerda < direita) {
        ...
        String pivo = lista[meio].estadoNascimento;
        String pivot = lista[meio].nome;
        ...
        while (i <= j) {
            comp++;
            while ((strcmp(lista[i].estadoNascimento, pivo) < 0) || ((strcmp(lista[i].estadoNascimento, pivo) == 0)&&(strcmp(lista[i].nome, pivot) < 0))) {
                i++;
                comp++;
            }
            while ((strcmp(lista[j].estadoNascimento, pivo) > 0) || ((strcmp(lista[j].estadoNascimento, pivo) == 0)&&(strcmp(lista[j].nome, pivot) > 0))) {
                j--;
                comp++;
            }
            comp++;
...
int main()
{
    ...
    FILE *escrever;
    escrever = fopen("00817611_quicksort.txt", "w");
    if (escrever == NULL) {
